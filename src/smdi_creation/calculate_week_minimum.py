"""
Weekly Minimum Soil Moisture → CSV
File: calculate_week_minimum.py
Author: James Nott
Date: 2025-08-11

Description:
    Computes the per-ISO-week (1–52) minimum soil moisture for every location in a
    large daily NetCDF cube. Handles missing values and filters out
    locations with insufficient data coverage. Designed to be memory-safe.
...
- CSV file where rows are locations and columns are week_01 … week_52 containing
  weekly minimum soil moisture values.

Usage:
python calculate_week_minimum.py <input.nc> <output.csv>
...
def write_weekly_minimums(nc_path: Path,
                         out_csv: Path,
                         loc_chunk: int = 500,
                         time_chunk: int = 365,
                         var_name: str = "sm") -> None:
    """Compute 52-week climatological minimums and save them to *out_csv*."""
    ...
    # 4. Weekly mean and Interpolation, then minimum across years  with all locations at once 
    # Returns NaN if there wasn't 5 data points to develop a minimum
    ...
    minimum_52wk = (
        weekly_mean.min("iso_year")               
                .where(valid_n >= 5)              # minimum but drops if < 5 values
    ) 

    # 5. Drops locations where there is less than 49/52 weeks

    # counts across the 52 weeks
    valid_per_loc = minimum_52wk.count("iso_week") 
     # keep only locations with at least 52 – 3 = 49 valid weeks

    needed = minimum_52wk.sizes["iso_week"] - 3             # 49
    loc_mask = valid_per_loc >= needed
    loc_mask = loc_mask.compute()
    minimum_52wk = minimum_52wk.where(loc_mask, drop=True)

    # num dropped
    dropped = int(ds.sizes["locations"] - minimum_52wk.sizes["locations"])
    print(f"Dropped {dropped} locations that had >3 NaNs in the 52-week record.")

    # 6. Bring the result to memory and convert to Pandas table

    table = minimum_52wk.compute().to_pandas()     # shape: (n_locations, 52)
    ...
    print(f"Saved climatological minimums to: {out_csv}")
