"""
Weekly Median Soil Moisture → CSV
File: calculate_week_median.py
Author: James Nott
Date: 2025-08-11

Description:
    Computes the per-ISO-week (1–52) median soil moisture for every location in a
    large daily NetCDF cube. Handles gaps and filters locations with insufficient
    coverage. Memory-safe for large datasets.
...
- CSV file where rows are locations and columns are week_01 … week_52 containing
  weekly median soil moisture values.

Usage:
python calculate_week_median.py <input.nc> <output.csv>
...
def write_weekly_medians(nc_path: Path,
                         out_csv: Path,
                         loc_chunk: int = 500,
                         time_chunk: int = 365,
                         var_name: str = "sm") -> None:
    """Compute 52-week climatological medians and save them to *out_csv*."""
    ...
    # 4. Weekly mean and Interpolation, then median across years with all locations at once 
    # Returns NaN if there wasn't 5 data points to develop a median
    ...
    median_52wk = (
        weekly_mean.median("iso_year")               
                .where(valid_n >= 5)              # Median but drops if < 5 values
    ) 
    ...
    needed = median_52wk.sizes["iso_week"] - 3             # 49
    ...
    table = median_52wk.compute().to_pandas()     # shape: (n_locations, 52)
...
if __name__ == "__main__":
    ...
    write_weekly_medians(args.nc_path,
                         args.out_csv)